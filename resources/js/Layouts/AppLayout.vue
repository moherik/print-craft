    ...
    <div class="min-h-screen flex flex-col bg-slate-50">
        ...
        <header v-if="!hideHeader" class="no-print sticky top-0 z-50 bg-white border-b border-slate-200">
                    ...
                    <div class="flex items-center gap-8">
                        ...
                        <div class="hidden md:flex items-center">
                            ...
                            <div class="relative" @mouseenter="showCategoryDropdown = true" @mouseleave="showCategoryDropdown = false">
                                ...
                                <div v-show="showCategoryDropdown" class="absolute top-full left-0 w-64 bg-white border border-slate-200 shadow-lg py-1">
                                    <Link
                                        v-for="cat in CATEGORIES"
                                        :key="cat.id"
                                        :href="route('categories.show', cat.slug)"
                                        class="flex items-center gap-3 p-3 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0"
                                    >
                                        <div class="w-8 h-8 flex items-center justify-center bg-slate-50">
                                            <component :is="cat.iconComponent" class="w-4 h-4 text-slate-500" />
                                        </div>
                                        <div>
                    ...
                    <div class="hidden md:flex items-center">
                        <form @submit.prevent="handleSearch" class="relative">
                            ...
                            <input
                                ...
                                class="w-64 pl-9 pr-3 py-1.5 bg-slate-50 border border-slate-200 rounded-none text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-red-600 focus:ring-0 transition-colors"
                            />
                        </form>
                    </div>
            ...
            <div v-show="showMobileMenu" class="md:hidden border-t border-slate-200 bg-white px-4 py-4 space-y-3 shadow-md">
                <form @submit.prevent="handleSearch" class="relative">
                    <Search class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                    <input v-model="searchQuery" type="text" placeholder="Cari template..." class="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 text-sm text-slate-900 focus:outline-none focus:border-red-600" />
                </form>
                <div class="pt-2">
        ...
        <footer v-if="!hideFooter" class="no-print border-t border-slate-200 bg-white mt-auto">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                    ...
                    <!-- Categories -->
                    <div>
                        <h4 class="font-bold text-slate-900 mb-3 text-sm uppercase tracking-wider">Kategori</h4>
                        <ul class="space-y-2">
